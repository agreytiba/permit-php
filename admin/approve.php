...
$sql = "SELECT *
        ...
        WHERE mainrequests.is_reviewed = 1 AND mainrequests.permit_status = 'reviewed' AND is_approved = 0
        LIMIT $starting_limit, $results_per_page";
$result = $conn->query($sql);

// Check if any rows were returned
if ($result->num_rows > 0) {
    // Initialize the request_d array
    $request_d = array();
    // Fetch associative array of rows
    while ($row = $result->fetch_assoc()) {
        // Append each row to the request_d array
        $request_d[] = $row;
    }
} else {
...
<h1 class="text-2xl text-center font-bold my-4 pt-4"> list of request under approve</h1>

<div class="p-5">
    <?php if (!empty($request_d)) : ?>
        <table class='table-auto border-collapse rounded shadow border border-green-800 min-w-full'>
            <thead>
            ...
            <tbody>
                <?php foreach ($request_d as $request) : ?>
                    <tr>
